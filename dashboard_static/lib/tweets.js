...
block.fn.tweets = function(config) {
    ...
    this.actions(function(e, tweet){
        ...
        // Adds an emergency / fire alarm icon
        let emerg_type = tweet.type
        if (emerg_type === 'firealarm'){
            $account.append($('<img class="icon" src="/media/fire.png" alt="Fire alarm icon">'));
        } else if (emerg_type === 'ambu'){
            $account.append($('<img class="icon" src="/media/ambu.png" alt="Ambulance icon">'));
        }

        // Adds a high / normal / low priority icon
        let priority = tweet.priority
        if (priority === 'high'){
            $account.append($('<img class="icon" src="/media/priohigh.png" alt="High priority icon">'));
        } else if (priority === 'normal'){
            $account.append($('<img class="icon" src="/media/prionormal.jpg" alt="Normal priority icon">'));
        } else if (priority === 'low'){
            $account.append($('<img class="icon" src="/media/priolow.png" alt="Low priority icon">'));
        }
