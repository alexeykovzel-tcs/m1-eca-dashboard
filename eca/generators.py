...
def offline_tweets(stop, data_file, time_factor=1000, arff_file=None):
    ...
    def delayer(duration):
        logger.debug("Delay for next tweet {}s ({}s real)".format(
            delay, delay/time_factor))
        return not stop.wait(duration / time_factor)
    ...
    with open(data_file, encoding='utf-8') as data:
        ...
        for line in data:
            ...
            try:
                ...
                if arff_file:
                    try:
                        ...
                        logger.error(
                            "Could not read arff line for tweet (reason: {})".format(e))
                        extra_data = None
                    tweet['extra'] = extra_data
            except ValueError as e:
                logger.error("Could not read tweet on {}:{} (reason: {})".format(
                    data_file, lines, e))
                continue
            ...
            # our team decided to increase the delay between tweets by 0.4 sec
            delay = wait.total_seconds() + 400

            emerg_code = dataconverter.get_emergency_code(tweet['text'])
            emerg_type = dataconverter.get_emergency_type(emerg_code)
            priority = dataconverter.get_emerg_code_priority(emerg_code)

            # adds an emergency type and priority into the tweet
            tweet['type'] = emerg_type
            tweet['priority'] = priority
...
def start_offline_tweets(data_file, event_name='tweet', aux_name='tweeter', **kwargs):
    ...
    if context is None:
        raise NotImplementedError(
            "Can not start offline tweet replay outside of a context.")
    register_auxiliary(aux_name, EventGenerator(
        context, generator=offline_tweets, data_file=data_file, event_name=event_name, **kwargs))
